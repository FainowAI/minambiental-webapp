...
import { createPhysicalChemicalAnalysis } from '@/services/contractMonitoringService';
...
const PhysicalChemicalAnalysisModal = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const parametrosExtras = {
      valoresReferencia: {
        temperaturaAgua: formData.valoresRefTemperaturaAgua || null,
        cor: formData.valoresRefCor || null,
        turbidez: formData.valoresRefTurbidez || null,
        ph: formData.valoresRefPh || null,
      },
      metodologias: {
        temperaturaAgua: formData.metodologiaTemperaturaAgua || null,
        cor: formData.metodologiaCor || null,
        turbidez: formData.metodologiaTurbidez || null,
        ph: formData.metodologiaPh || null,
      },
    };
    ...
    const submit = async () => {
      try {
        ...
        await createPhysicalChemicalAnalysis({
          contractId,
          createdBy: user?.id,
          responsavelColeta: formData.responsavelColeta,
          indProfissional: formData.indProfissional,
          laboratorio: formData.laboratorio,
          dataEntradaLaboratorio: formData.dataEntradaLaboratorio,
          dataColeta: formData.dataColeta,
          horaColeta: formData.horaColeta,
          temperaturaAmbiente: parseNumber(formData.temperaturaAmbiente),
          temperaturaAmostra: parseNumber(formData.temperaturaAmostra),
          condicoesTempo: formData.condicoesTempo,
          codigoAmostra: formData.codigoAmostra,
          resultadoTemperaturaAgua: parseNumber(formData.resultadoTemperaturaAgua),
          resultadoCor: parseNumber(formData.resultadoCor),
          resultadoTurbidez: parseNumber(formData.resultadoTurbidez),
          resultadoPh: parseNumber(formData.resultadoPh),
          parametrosExtras,
        });
        ...
        const message = error instanceof Error ? error.message : 'Erro desconhecido ao salvar a análise.';
        toast({
