...
import { createNdNeEntries } from '@/services/contractMonitoringService';
...
const NDNEModal = ({ isOpen, onClose, contractId, onSuccess }: NDNEModalProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const submit = async () => {
      try {
        ...
        await createNdNeEntries({
          contractId,
          createdBy: user?.id,
          entries: [
            {
              periodo: 'seca',
              nivelEstatico: parseNumber(formData.neSecaMetros),
              nivelDinamico: parseNumber(formData.ndSecaMetros),
            },
            {
              periodo: 'chuva',
              nivelEstatico: parseNumber(formData.neChuvaMetros),
              nivelDinamico: parseNumber(formData.ndChuvaMetros),
            },
          ],
        });
        ...
        const message = error instanceof Error ? error.message : 'Erro desconhecido ao registrar os níveis.';
        toast({
          title: 'Erro ao registrar ND/NE',
          description: message,
          variant: 'destructive',
        });
      } finally {
      ...
      <DialogContent className="max-w-3xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-4">
          ...
          <DialogFooter className="gap-2">
            ...
            >
              {isSubmitting ? 'Salvando...' : 'Salvar Níveis'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
