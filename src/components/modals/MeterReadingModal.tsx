...
import { createMeterReading } from '@/services/contractMonitoringService';
...
const MeterReadingModal = ({ isOpen, onClose, contractId, onSuccess }: MeterReadingModalProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const now = new Date();
    const referenciaAno = now.getFullYear();
    const referenciaMes = now.getMonth() + 1;
    ...
    const submit = async () => {
      try {
        ...
        await createMeterReading({
          ...
          referenciaMes,
          dataLeitura,
          leituraDeclarada: parseNumber(formData.leituraDeclarada),
          horaDeclarada: parseNumber(formData.horaDeclarada),
          leituraApurada: Number(formData.leituraApurada),
          horaApurada: Number(formData.horaApurada),
          observacoes: undefined,
          images: images.map((image) => ({ file: image.file })),
        });
        ...
        const message = error instanceof Error ? error.message : 'Erro desconhecido ao registrar as leituras.';
        toast({
          title: 'Erro ao salvar leituras',
          description: message,
          variant: 'destructive',
        });
      } finally {
