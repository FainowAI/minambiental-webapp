...
import { maskCEP, maskCurrency, unmaskCurrency, maskCPF, maskPhone } from '@/utils/masks';
...
import { createContract, updateRequerenteContatoMedicao } from '@/services/contractService';
...
const CreateContract = () => {
  ...
  const navItems = [
    ...
    },
    {
      ...
      icon: FileText,
      url: '/licenses',
      isActive: location.pathname === '/licenses' || location.pathname.startsWith('/create-contract'),
    },
    {
      title: 'Usuários',
      icon: Users,
  ...
  useEffect(() => {
    const loadLicense = async () => {
      ...
      try {
        ...
        // Pre-populate contact for measurement if requerente has it
        if (license.requerente) {
          // Try to get full user data to check for contato_medicao fields
          try {
            ...
            // If we can't get user data, just continue without pre-populating
            console.log('Could not load requerente contact data:', err);
          }
        }
      } catch (error) {
  ...
  // CEP Lookup for Contract Address
  const handleLookupCEPContrato = async () => {
    if (!validateCEP(formData.cepContrato)) {
      setErrors((e) => ({ ...e, cepContrato: 'CEP inválido' }));
      toast({
        title: 'CEP inválido',
    ...
    try {
      const addressData = await lookupCEP(formData.cepContrato);
  ...
  // CEP Lookup for Construction/Work Address
  const handleLookupCEPObra = async () => {
    if (!validateCEP(formData.cepObra)) {
      setErrors((e) => ({ ...e, cepObra: 'CEP inválido' }));
      toast({
        title: 'CEP inválido',
        description: 'Verifique o CEP informado.',
        variant: 'destructive',
    ...
    try {
      const addressData = await lookupCEP(formData.cepObra);
  ...
  const handleLookupTechnician = async () => {
    ...
    try {
      // Search for users with perfil "Técnico" - we'll need to query Supabase
      // For now, we'll use a simple approach
      const { supabase } = await import('@/integrations/supabase/client');
      const cleanCPF = formData.cpfTecnico.replace(/\D/g, '');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        tipoContratante: formData.tipoContratante,
        vinculoArt: formData.vinculoArt || undefined,
        acaoInstitucional: formData.acaoInstitucional || undefined,

        cepContrato: formData.cepContrato.replace(/\D/g, ''),
        ruaContrato: formData.ruaContrato,
        bairroContrato: formData.bairroContrato,
        numeroContrato: formData.numeroContrato,
        cidadeContrato: formData.cidadeContrato,
        estadoContrato: formData.estadoContrato,
        paisContrato: formData.paisContrato,

        cepObra: formData.cepObra.replace(/\D/g, ''),
        ruaObra: formData.ruaObra,
        bairroObra: formData.bairroObra,
        numeroObra: formData.numeroObra,
        cidadeObra: formData.cidadeObra,
        estadoObra: formData.estadoObra,
        paisObra: formData.paisObra,
        ...
        cpfContato: formData.cpfContato.replace(/\D/g, ''),
        ...
        cpfTecnico: formData.cpfTecnico.replace(/\D/g, ''),
        nomeTecnico: formData.nomeTecnico,
        rnp: formData.rnp,
        tituloProfissional: formData.tituloProfissional,
        registroEmpresaContratada: formData.registroEmpresaContratada,
      ...
      // Update requerente contact for measurement
      ...
      toast({
        ...
        description: error.message || 'Ocorreu um erro ao salvar o contrato. Tente novamente.',
        variant: 'destructive',
      });
    } finally {
      setIsSaving(false);
  ...
  return (
    <SidebarProvider>
            ...
            <div className="flex items-center gap-2">
              ...
              <Breadcrumb>
                <BreadcrumbList>
                  ...
                  <BreadcrumbSeparator />
                  <BreadcrumbItem>
                    <BreadcrumbPage>Cadastrar Contrato</BreadcrumbPage>
                  </BreadcrumbItem>
                </BreadcrumbList>
              </Breadcrumb>
            </div>
            ...
            >
              ...
              <div className="p-6 md:p-8">
                <form className="space-y-8">
                      ...
                      <div className="space-y-2">
                        ...
                        <Select
                          ...
                          onValueChange={(value: any) =>
                            updateFormField('tipoContratante', value)
                          }
                        >
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            onClick={handleLookupCEPContrato}
                          >
                  ...
                  {/* SEÇÃO 3: Contato para Medição de Hidrômetro e Horímetro */}
                  <div>
                    <div className="mb-6">
                      <h3 className="text-lg font-semibold text-gray-800">
                    ...
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            updateFormField('periodoMedicaoInicio', e.target.value)
                          }
                          className="h-11 border-gray-300"
                        />
                  ...
                  <div>
                    ...
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                      ...
                      {/* Registro da Empresa Contratada */}
                      <div className="space-y-2 lg:col-span-2">
                        <Label
      ...
      {/* Dialog: Technician Not Found */}
      <AlertDialog
        open={showTechnicianNotFoundDialog}
        onOpenChange={setShowTechnicianNotFoundDialog}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Técnico não encontrado</AlertDialogTitle>
            <AlertDialogDescription>
              O CPF informado não foi encontrado no sistema ou não possui perfil de Técnico. Por
              favor, realize o cadastro do técnico antes de vinculá-lo como responsável no
              contrato.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
